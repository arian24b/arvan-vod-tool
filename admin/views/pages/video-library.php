...
require_once plugin_dir_path(dirname(dirname(__FILE__))) . 'includes/class-arvancloud-vod-api.php';
...
// Check if connection is working and display appropriate message
if (!$api->test_connection()) {
    ?>
    <div class="notice notice-error">
        <p><?php _e('Cannot connect to ArvanCloud VOD API. Please check your API key in the settings.', 'arvancloud-vod'); ?></p>
    </div>
    <?php
}
...
if ( isset( $_REQUEST['s'] ) && strlen( $_REQUEST['s'] ) ) {
	...
	printf(
		...
		__( 'Search results for: %s', 'arvancloud-vod' ),
...
if ( ! empty( $_GET['posted'] ) ) {
	...
	$_SERVER['REQUEST_URI'] = remove_query_arg( array( 'posted' ), wp_unslash( $_SERVER['REQUEST_URI'] ) );
}
...
if ( ! empty( $_GET['attached'] ) && absint( $_GET['attached'] ) ) {
	...
	$_SERVER['REQUEST_URI'] = remove_query_arg( array( 'detach', 'attached' ), wp_unslash( $_SERVER['REQUEST_URI'] ) );
}
...
if ( ! empty( $_GET['trashed'] ) && absint( $_GET['trashed'] ) ) {
	...
	$_SERVER['REQUEST_URI'] = remove_query_arg( array( 'trashed' ), wp_unslash( $_SERVER['REQUEST_URI'] ) );
}
...
if ( ! empty( $_GET['untrashed'] ) && absint( $_GET['untrashed'] ) ) {
	...
	$_SERVER['REQUEST_URI'] = remove_query_arg( array( 'untrashed' ), wp_unslash( $_SERVER['REQUEST_URI'] ) );
}
...
if ( ! empty( $_GET['message'] ) && isset( $messages[ $_GET['message'] ] ) ) {
	...
	$_SERVER['REQUEST_URI'] = remove_query_arg( array( 'message' ), wp_unslash( $_SERVER['REQUEST_URI'] ) );
}
...
<?php
// Check if $videos variable exists and is empty
$videos = isset($videos) ? $videos : [];
if (empty($videos)) {
    ?>
    <div class="notice notice-warning">
        <p><?php _e('No videos found. If you believe this is an error, try syncing videos again.', 'arvancloud-vod'); ?></p>
        <p><a href="<?php echo esc_url(admin_url('admin.php?page=arvancloud-vod-videos-sync')); ?>" class="button"><?php _e('Sync Videos', 'arvancloud-vod'); ?></a></p>
    </div>
    <?php
}
?>
